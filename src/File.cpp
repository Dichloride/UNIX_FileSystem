...
void Show_File_List(bool detail)
{
	...
	cout << setw(time_length) << "Edit Time" << setw(type_length) << "Type" << setw(size_length) << "Size(Byte)" << setw(file_name_length) << "File/Directory Name";
	if (detail)
	{
		cout << setw(user_id_length) << "Owner Id" << setw(user_group_length) << "Owner Group" << setw(inode_id_length) << "Inode Id" << setw(permission_length) << "O[RWE]G[RWE]E[RWE]";
	}
	...
	for (int i = 0; i < SUBDIRECTORY_NUM; i++)
	{
		if (strlen(directory.d_filename[i]) > 0)
		{
			...
			if (detail)
			{
				...
				cout << setw(user_id_length) << inode.i_uid << setw(user_group_length) << inode.i_gid << setw(inode_id_length) << inode.i_number << setw(permission_length) << tmp;
			}
			cout << endl;
		}
	}
